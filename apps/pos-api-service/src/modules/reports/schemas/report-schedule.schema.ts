...
/**
 * Zod schema for creating a new automated report schedule.
 * The cron service reads active schedules and emails reports on each run.
 */
export const CreateReportScheduleSchema = z.object({
    scheduleName: z
        .string()
        .min(SCHEDULE_NAME_MIN, `Schedule name must be at least ${SCHEDULE_NAME_MIN} characters`)
        .max(SCHEDULE_NAME_MAX, `Schedule name must not exceed ${SCHEDULE_NAME_MAX} characters`)
        .trim(),
...
/**
 * Zod schema for toggling a schedule's active state (PATCH).
 */
export const UpdateScheduleStatusSchema = z.object({
    isActive: z.boolean(),
});

export type UpdateScheduleStatusDto = z.infer<typeof UpdateScheduleStatusSchema>;