...
export class ProductController {
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: 'name / SKU / barcode' })
  @ApiQuery({ name: 'status', required: false, enum: ['ACTIVE', 'INACTIVE', 'DISCONTINUED'] })
  @ApiQuery({ name: 'categoryId', required: false, type: String })
  @ApiQuery({ name: 'supplierId', required: false, type: String })
  @ApiResponse({ status: 200, description: 'Paginated product list' })
  findAll(@Query(new ZodValidationPipe(ProductListQuerySchema)) query: ProductListQueryDto) {
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        costPrice:   { type: 'number', example: 180.00 },
        minStock:    { type: 'integer', example: 5 },
        unit:        { type: 'string', enum: ['PCS', 'KG', 'PACK', 'LTR', 'BOX', 'MTR'], example: 'PCS' },
        status:      { type: 'string', enum: ['ACTIVE', 'INACTIVE', 'DISCONTINUED'], example: 'ACTIVE' },
        categoryId:  { type: 'string', format: 'uuid', example: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx' },
        supplierId:  { type: 'string', format: 'uuid', example: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx' },
      },
    },
  })
