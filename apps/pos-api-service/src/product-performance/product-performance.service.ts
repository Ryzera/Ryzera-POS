...
/**
 * Raw row returned by the Prisma saleItem query.
 * FIX: quantity/total_amount/cost_price are Decimal in the schema — use number
 * after converting with Number() at query boundaries.
 */
type SaleItemRow = {
  product_name: string;
  quantity: number;
...
export class ProductPerformanceService {
  ...
  private buildSaleItemWhere(filter: ResolvedProductPerformanceFilter): object {
    ...
    if (filter.resolvedBranchId !== undefined) {
      saleWhere['branch_id'] = filter.resolvedBranchId;
    }
  ...
  private buildPaymentWhere(filter: ResolvedProductPerformanceFilter): object {
    const dateFrom = filter.dateFrom
        ? new Date(filter.dateFrom)
        : DEFAULT_DATE_FROM;
    const dateTo = filter.dateTo
    ...
    if (filter.resolvedBranchId !== undefined) {
      where['sale'] = { branch_id: filter.resolvedBranchId };
    }
    ...
    for (const item of items) {
      ...
      if (!grouped[name]) {
        grouped[name] = {
          ...
          // FIX: unit lives on item.product.unit — read it there
      ...
      // FIX: already converted to number before entering this function
  ...
  /**
   * Convert Prisma saleItem rows (which have Decimal fields) into SaleItemRow
   * (which uses plain numbers) so all arithmetic is type-safe.
   */
  private toSaleItemRows(
      items: Array<{
        product_name: string;
        quantity: any;
        total_amount: any;
  ...
  async getKpiCards(filter: ResolvedProductPerformanceFilter) {
    ...
    const rawItems = await this.prisma.saleItem.findMany({
      where,
      select: {
        product_name: true,
        quantity: true,
    ...
    const items = this.toSaleItemRows(rawItems);
    ...
    for (const item of items) {
      // FIX: all Decimal values already converted to number in toSaleItemRows
      ...
      productQtyMap[productName] =
          (productQtyMap[productName] ?? 0) + item.quantity;
      categoryQtyMap[categoryName] =
          (categoryQtyMap[categoryName] ?? 0) + item.quantity;
    }

    const topSellingProduct =
        Object.entries(productQtyMap).sort(([, a], [, b]) => b - a)[0]?.[0] ??
        'N/A';

    const topSellingCategory =
        Object.entries(categoryQtyMap).sort(([, a], [, b]) => b - a)[0]?.[0] ??
        'N/A';
  ...
  async getTopProducts(filter: ResolvedProductPerformanceFilter) {
    ...
    for (const item of rawItems) {
      grouped[item.product_name] =
          (grouped[item.product_name] ?? 0) + Number(item.quantity);
    }
  ...
  async getPaymentMethodBreakdown(filter: ResolvedProductPerformanceFilter) {
    ...
    const matchingItems = await this.prisma.saleItem.findMany({
      where: saleItemWhere,
      // FIX: select sale_id (snake_case — actual Prisma field)
      select: { sale_id: true },
    });
    ...
    const saleIds = [...new Set(matchingItems.map((i) => i.sale_id))];

    const payments = await this.prisma.payment.findMany({
      // FIX: filter by sale_id (snake_case)
    ...
    for (const payment of payments) {
      ...
      grouped[method].totalAmount += Number(payment.amount_paid);
    }
    ...
    const data = Object.entries(grouped).map(([paymentMethod, values]) => ({
      paymentMethod,
      count: values.count,
  ...
  async getPerBranchSummary(filter: ResolvedProductPerformanceFilter) {
    const branches = await this.prisma.branch.findMany({
      ...
      // FIX: select 'id' — the Prisma field name (mapped from branch_id column)
    ...
    const result = await Promise.all(
        branches.map(async (branch) => {
          const branchFilter: ResolvedProductPerformanceFilter = {
            ...filter,
            // FIX: use branch.id (not branch.branchId — that's the column alias)
          ...
          for (const item of rawItems) {
            ...
            qtyMap[item.product_name] =
                (qtyMap[item.product_name] ?? 0) + Number(item.quantity);
            revenueMap[item.product_name] =
                (revenueMap[item.product_name] ?? 0) + Number(item.total_amount);
          }
          ...
          // FIX: use sale_id (snake_case)
          const branchSaleIds = [...new Set(rawItems.map((i) => i.sale_id))];

          const payments =
              branchSaleIds.length === 0
                  ? []
                  : await this.prisma.payment.findMany({
                    // FIX: use sale_id (snake_case)
                    where: { sale_id: { in: branchSaleIds } },
                    select: { payment_method: true, amount_paid: true },
                  });
          ...
          for (const p of payments) {
            ...
            pmGrouped[method].totalAmount += Number(p.amount_paid);
          }
  ...
  async getProductTable(filter: ResolvedProductPerformanceFilter) {
    ...
    // FIX: convert Decimal fields before grouping
    const items = this.toSaleItemRows(rawItems);
  ...
  async exportToCsv(
    ...
    void this.auditLogService.record({
      ...
      branchName: filter.resolvedBranchId
          ? `Branch ${filter.resolvedBranchId}`
          : 'All',
      branchId: filter.resolvedBranchId ?? null,
    });
