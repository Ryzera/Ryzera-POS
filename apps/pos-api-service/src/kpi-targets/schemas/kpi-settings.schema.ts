...
// ─────────────────────────────────────────────────────────────────────────────
export const UpsertMarginTargetSchema = z
  .object({
    /**
  ...
  .refine(
    ...
    {
      message:
        'target_net_margin must be less than or equal to target_gross_margin',
...
//     Field names match the UI labels exactly.
// ─────────────────────────────────────────────────────────────────────────────
export const UpsertInventoryThresholdSchema = z
  .object({
    /**
  ...
  .refine(
    ...
    {
      message:
        'critical_stock_level must be strictly less than default_reorder_level',
      path: ['critical_stock_level'],
    },
  );

export type UpsertInventoryThresholdDto = z.infer<
  typeof UpsertInventoryThresholdSchema
>;
...
export type UpsertNotificationRuleDto = z.infer<
  typeof UpsertNotificationRuleSchema
>;
...
export const SaveAllKpiSettingsSchema = z.object({
   ...
   */
  sales_targets: z
    .array(UpsertSalesTargetSchema)
    .min(1, 'At least one sales target is required'),
