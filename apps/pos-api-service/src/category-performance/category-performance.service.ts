...
export class CategoryPerformanceService {
    ...
    /**
     * Builds the Prisma WHERE clause for branch filtering.
     *
     * Uses strict `!== undefined` check (not a falsy check) so that:
     *   branchId = undefined  →  no clause  →  ALL branches queried  (All Branches tab)
     *   branchId = N          →  WHERE branchId = N                  (single branch)
     *
     * IMPORTANT: Do NOT use `!branchId` or `branchId == null` — branchId = 0 is
     * falsy and would incorrectly bypass the filter. Always use `!== undefined`.
     */
    ...
    /**
     * Parses dateFrom / dateTo strings from the DTO into Date objects.
     * dateTo is pushed to end-of-day so all sales on that date are included.
     */
    ...
    private async fetchCompletedSales(dto: QueryCategoryPerformanceDto) {
        ...
        return this.prisma.ryzera_pos_sale.findMany({
            where: {
                ...
                ...this.buildBranchFilter(dto.branchId),
            },
            include: {
                sale_items: {
                    include: {
    ...
    private aggregateByCategory(
        ...
        for (const sale of sales) {
            for (const item of sale.sale_items) {
                ...
                if (item.productId) grouped[cat].products.add(item.productId);
                grouped[cat].totalSold    += item.quantity;
                grouped[cat].revenue      += Number(item.total_amount);
                grouped[cat].cost         += Number(item.cost_price) * item.quantity;
                grouped[cat].transactions.add(sale.sale_id);
            }
        }
    ...
    async getProfitByCategory(dto: QueryCategoryPerformanceDto) {
        ...
        const total   = Object.values(grouped).reduce(
            (sum, v) => sum + (v.revenue - v.cost), 0,
        );

        const data = Object.entries(grouped).map(([category, v]) => {
            const profit     = v.revenue - v.cost;
            const percentage = total > 0
                ? parseFloat(((profit / total) * 100).toFixed(1))
                : 0;
            return {
                category,
    ...
    async getCategoryPerformanceByBranch(dto: QueryCategoryPerformanceDto) {
        ...
        const results = await Promise.all(
            branches.map(async (branch) => {
                ...
                const branchDto   = { ...dto, branchId: branch.branchId };
                const tableResult = await this.getCategoryTable(branchDto);
                return {
                    branch: { id: branch.branchId, name: branch.name },
    ...
    async exportCsv(dto: QueryCategoryPerformanceDto): Promise<Buffer> {
        ...
        if (!result.data || result.data.length === 0) {
            return Buffer.from('No data available for the selected date range.', 'utf-8');
        }

        const headers = [
            'Category', 'Total Products', 'Total Sold',
            'Revenue (LKR)', 'Cost (LKR)', 'Profit (LKR)', 'Transactions', 'Margin (%)',
        ];

        const rows = result.data.map((r) => [
            r.category, r.totalProducts, r.totalSold,
    ...
    async exportPdf(dto: QueryCategoryPerformanceDto): Promise<Buffer> {
        ...
        return new Promise((resolve) => {
            ...
            const dateLabel   = dto.dateFrom && dto.dateTo
                ? `${dto.dateFrom}  →  ${dto.dateTo}`
                : 'All dates';

            doc.fontSize(16).font('Helvetica-Bold').text(reportTitle, { align: 'center' });
            doc.fontSize(9).font('Helvetica').text(`Date range: ${dateLabel}`, { align: 'center' });
            doc.moveDown(0.8);
            ...
            if (rows.length === 0) {
                doc.fontSize(11).text('No data available for the selected date range.', { align: 'center' });
                doc.end();
                return;
            }
                ...
                cols.forEach((col, i) => {
                    ...
                    doc.font(isHeader ? 'Helvetica-Bold' : 'Helvetica')
                        ...
                        .text(
                            text,
                            x + 4,
                            y + (isHeader ? headerHeight : rowHeight) / 2 - (isHeader ? 4.25 : 4),
            ...
            // ── Column header row ────────────────────────────────────────────
            drawRow(cols.map((c) => c.label), curY, true);
            curY += headerHeight;
            ...
            const totals = rows.reduce(
                ...
                { totalProducts: 0, totalSold: 0, revenue: 0, cost: 0, profit: 0, transactions: 0 },
            );
            ...
            // Totals background
            doc.rect(startX, curY, cols.reduce((s, c) => s + c.width, 0), rowHeight)
                .fill('#1E3A5F');
            ...
            cols.forEach((col, i) => {
                doc.font('Helvetica-Bold')
                    ...
                    .fillColor('#FFFFFF')
                    .text(
                        String(totalValues[i]),
                        tx + 4,
                        curY + rowHeight / 2 - 4.25,
                        { width: col.width - 8, align: col.align, lineBreak: false },
                    );
                tx += col.width;
            });
