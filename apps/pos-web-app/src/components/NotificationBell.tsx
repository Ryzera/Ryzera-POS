...
interface NotificationBellProps {
  mode?: 'BROADCAST_ONLY' | 'ALL';
}

export default function NotificationBell({ mode = 'BROADCAST_ONLY' }: NotificationBellProps) {
  ...
  const fetchNotifs = async () => {
    try {
      ...
      if (nr.ok) {
        ...
        const allNotifs: Notification[] = Array.isArray(d) ? d : (Array.isArray(d?.data) ? d.data : []);
        
        // When in Main Dashboard (BROADCAST_ONLY mode):
        // Show general Broadcast Announcements, Staff Notices, and Business Alerts.
        // Filter out deep IT Sync Engine logs (like retry attempts, circuit breaker logs).
        const fetchedNotifs = mode === 'BROADCAST_ONLY'
          ? allNotifs.filter(n => {
              const title = (n.title || '').toLowerCase();
              const msg = (n.message || '').toLowerCase();
              const isTechSyncLog = title.includes('sync failed') || title.includes('circuit') || title.includes('sync retry');
              return !isTechSyncLog;
            })
          : allNotifs;
