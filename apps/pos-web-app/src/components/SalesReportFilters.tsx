...
export function SalesReportFilters({
  ...
  const [dateFrom, setDateFrom] = useState<Date>(() =>
    paramDateFrom ? new Date(paramDateFrom) : new Date(DEFAULT_DATE_FROM),
  );
  const [dateTo, setDateTo] = useState<Date>(() =>
    paramDateTo ? new Date(paramDateTo) : new Date(DEFAULT_DATE_TO),
  );
  const [categoryId, setCategoryId] = useState<string>(
    () => paramCategoryId || ALL_CATEGORIES_VALUE,
  );
  const [productId, setProductId] = useState<string>(
    () => paramProductId || ALL_PRODUCTS_VALUE,
  );
  ...
  // Auto-generate if navigated from Saved Configs with URL parameters
  useEffect(() => {
    if (paramDateFrom || paramDateTo || paramCategoryId) {
      onGenerate({
        dateFrom: paramDateFrom || format(dateFrom, "yyyy-MM-dd"),
        dateTo: paramDateTo || format(dateTo, "yyyy-MM-dd"),
        categoryId:
          paramCategoryId && paramCategoryId !== ALL_CATEGORIES_VALUE
            ? paramCategoryId
            : undefined,
        productId:
          paramProductId && paramProductId !== ALL_PRODUCTS_VALUE
            ? paramProductId
            : undefined,
      });
    }
  }, [paramDateFrom, paramDateTo, paramCategoryId, paramProductId]);
  ...
  const handleGenerate = () => {
    ...
    const selectedCategory = categoriesRef.current.find(
    ...
    onGenerate({
      ...
      categoryId:
        currentCategoryId !== ALL_CATEGORIES_VALUE
          ? selectedCategory?.name
          : undefined,
      productId:
        currentProductId !== ALL_PRODUCTS_VALUE
          ...
          >
            <Calendar
              ...
              classNames={{
                ...
                nav_button:
                  "h-7 w-7 bg-transparent hover:bg-gray-100 rounded-lg flex items-center justify-center transition-colors",
                nav_button_previous: "absolute left-1",
                nav_button_next: "absolute right-1",
                table: "w-full border-collapse",
                head_row: "flex",
                head_cell:
                  "text-gray-400 rounded-md w-9 font-medium text-[11px] text-center",
                ...
                day_selected:
                  "bg-blue-600 text-white hover:bg-blue-600 hover:text-white font-semibold rounded-lg",
