...
import { DEFAULT_SUMMARY_DATE } from "@/constants/daily-summary.constants";
import type { DailySummaryFilters } from "@/types/daily-summary.types";

interface DailySummaryFiltersProps {
  onGenerate: (filters: DailySummaryFilters) => void;
  isLoading: boolean;
}

export function DailySummaryFilters({
  onGenerate,
  isLoading,
}: DailySummaryFiltersProps) {
  const searchParams = useSearchParams();
  const paramDate = searchParams?.get("date") || searchParams?.get("dateFrom");

  const [selectedDate, setSelectedDate] = useState<Date>(() =>
    paramDate ? new Date(paramDate) : new Date(DEFAULT_SUMMARY_DATE),
  );

  // Auto-generate if navigated from Saved Configs
  useEffect(() => {
    if (paramDate) {
      onGenerate({ date: paramDate });
    }
  }, [paramDate]);
  ...
  const handleGenerate = () => {
    onGenerate({ date: format(selectedDate, "yyyy-MM-dd") });
  };

  const handleReset = () => {
    setSelectedDate(new Date(DEFAULT_SUMMARY_DATE));
  };
      ...
      <div className="flex flex-col gap-1">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className={cn(triggerClass, "w-52")}>
