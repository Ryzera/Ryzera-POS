...
import { Search, Printer, Eye, FileText, RefreshCw } from 'lucide-react';
...
export default function ReceiptsPage() {
    const { sales: localSales } = useSalesStore();
    ...
    async function fetchReceipts() {
        ...
        try {
            ...
            const normalized: Sale[] = list
                .filter((s: any) => (s.status || 'COMPLETED').toUpperCase() !== 'CANCELLED')
                .map((s: any) => {
                    ...
                    return {
                        ...
                        cashier_name: s.cashier?.info
                            ? `${s.cashier.info.first_name} ${s.cashier.info.last_name || ''}`.trim()
                            : (s.cashier?.username || 'Cashier'),
                        items,
                    };
                });
    ...
    // Combine DB sales with local sales
    const completedSales: Sale[] = (() => {
        const dbInvoiceSet = new Set(dbSales.map(s => s.invoice_no));
        const uniqueLocal = localSales.filter(s => s.status === 'completed' && !dbInvoiceSet.has(s.invoice_no));
        ...
        <>
            ...
            {selected && (
                <div id="receipt-print-area" style={{ padding: '32px', maxWidth: '320px', margin: '0 auto', fontFamily: 'monospace', fontSize: '13px' }}>
                    ...
                    <hr style={{ borderStyle: 'dashed', borderColor: '#9ca3af', margin: '8px 0' }} />
                    {(selected.discount_amount || 0) > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}><span>Discount</span><span>- Rs. {selected.discount_amount.toLocaleString()}</span></div>}
                    {(selected.tax_amount || 0) > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}><span>Tax</span><span>Rs. {selected.tax_amount.toLocaleString()}</span></div>}
                    ...
                    <div style={{ flex: 1, background: '#fff', borderRadius: '16px', border: '1px solid #e5e7eb', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto' }}>
                            {loading ? (
                                ...
                                const pmStyle = PAYMENT_STYLE[receipt.payment_method] ?? { background: '#f3f4f6', color: '#4b5563' };
                                return (
                                    <div
                                        key={receipt.id}
                        ...
                        <>
                            <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2px' }}>
                                    ...
                                    <span style={{ padding: '2px 8px', borderRadius: '6px', fontSize: '11px', fontWeight: 600, ...(PAYMENT_STYLE[selected.payment_method] ?? { background: '#f3f4f6', color: '#4b5563' }) }}>{selected.payment_method}</span>
                                </div>
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
                                ...
                                <div style={{ background: '#f9fafb', borderRadius: '12px', padding: '16px', fontFamily: 'monospace', fontSize: '12px' }}>
                                    ...
                                    <hr style={{ borderStyle: 'dashed', borderColor: '#d1d5db', margin: '8px 0' }} />
                                    {(selected.discount_amount || 0) > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', color: '#16a34a' }}><span>Discount</span><span>- Rs.{selected.discount_amount.toLocaleString()}</span></div>}
                                    {(selected.tax_amount || 0) > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', color: '#6b7280' }}><span>Tax</span><span>Rs.{selected.tax_amount.toLocaleString()}</span></div>}
                            ...
                            <div style={{ padding: '12px 20px 20px', flexShrink: 0 }}>
                                <button
                                    ...
                                    style={{ width: '100%', height: '40px', border: 'none', borderRadius: '8px', background: '#2563eb', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
