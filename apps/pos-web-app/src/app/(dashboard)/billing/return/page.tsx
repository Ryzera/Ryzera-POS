...
import api, { extractArray, processReturn } from '@/lib/api';
...
export default function ReturnPage() {
    ...
    async function handleSearch() {
        const query = invoiceInput.trim().toUpperCase();
        if (!query) return;
        ...
        try {
            ...
            try {
                ...
                const match = list.find((s: any) => {
                    const inv = (s.invoice_number || s.invoice_no || s.bill_number || `INV-${s.sale_id || s.id}`).toUpperCase();
                    return inv === query;
                });

                if (match) {
                    const invoice = match.invoice_number || match.invoice_no || match.bill_number || `INV-${match.sale_id || match.id}`;
                    const total = Number(match.grand_total ?? match.total_amount ?? match.total ?? 0);
                    const discount = Number(match.discount_amount ?? match.discount_total ?? 0);
                    const tax = Number(match.tax_amount ?? match.tax_total ?? 0);
                    const rawStatus = (match.status || match.sale_status || 'COMPLETED').toLowerCase();

                    const rawItems = match.saleItems || match.items || match.sale_items || [];
                    const items = Array.isArray(rawItems) ? rawItems.map((it: any, idx: number) => ({
                        id: it.id ?? it.sale_item_id ?? idx + 1,
                    ...
                    foundSale = {
                        id: match.sale_id ?? match.id,
                        invoice_no: invoice,
                        total_amount: total,
                        discount_amount: discount,
                        tax_amount: tax,
                        payment_method: match.payment_method || 'Cash',
                        status: rawStatus as any,
                        created_at: match.created_at || match.createdAt || new Date().toISOString(),
                        cashier_name: match.cashier?.info
                            ? `${match.cashier.info.first_name} ${match.cashier.info.last_name || ''}`.trim()
                            : (match.cashier?.username || 'Cashier'),
                        items,
                    };
                }
            } catch {
            ...
            if (!foundSale) {
                const localMatch = localSales.find(s => s.invoice_no.toUpperCase() === query);
                if (localMatch) foundSale = localMatch;
            }
            ...
            setError('Failed to search for invoice.');
        } finally {
            setLoading(false);
        }
    }
    ...
    async function handleProcessReturn() {
        ...
        try {
            ...
            await processReturn({
                sale_id:       sale.id,
                reason:        reason.trim(),
                refund_method: refundMethod.toUpperCase(),
