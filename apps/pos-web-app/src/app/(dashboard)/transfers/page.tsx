...
interface TransferItem { id: string; quantity: number; product?: { name: string; sku: string; unit: string }; }

// CASCADE: include status so we can filter active-only in dropdowns
interface Branch  { id: string; name: string; status?: string; }
interface Product { id: string; name: string; sku: string; unit: string; status?: string; }
interface StockItem { productId: string; productName: string; branchId: string; quantity: number; unit?: string; }
...
export default function TransfersPage() {
    ...
    const getAvailableStock = (productId: string, branchId: string) => {
        const item = stockItems.find(s => s.productId === productId && s.branchId === branchId);
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                sourceBranchId:      createForm.sourceBranchId || userBranchId,
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select value={createForm.destinationBranchId}
                            ...
                            {activeBranches
                                .filter(b => b.id !== (createForm.sourceBranchId || userBranchId))
                                .map(b => (
                                    <option key={b.id} value={b.id}>{b.name}</option>
                                ))}
                        </select>
                        {activeBranches.filter(b => b.id !== (createForm.sourceBranchId || userBranchId)).length === 0 && (
                            ...
                            <tr key={idx} className="border-b border-gray-50">
                                ...
                                <td className="py-2 pr-4 text-gray-500 text-sm">
                                    {item.productId && (createForm.sourceBranchId || userBranchId)
                                        ? getAvailableStock(item.productId, createForm.sourceBranchId || userBranchId!)
                                        : "—"}
                                </td>
                                <td className="py-2 pr-4">
                ...
                <div>
                    <p className="text-sm text-[#4A8FD4] mb-1 cursor-pointer hover:underline"
                       onClick={() => setView("list")}>
                        Transfers / {selected.id.slice(0, 8).toUpperCase()}
                    </p>
                    <h1 className="text-xl font-semibold text-gray-900">
                        Transfer #{selected.id.slice(0, 8).toUpperCase()}
                    </h1>
                </div>
