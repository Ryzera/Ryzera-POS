...
function normalizeProduct(p: any): Product {
    return {
        ...
        supplierId: String(p.supplierId ?? p.supplier_id ?? p.supplier?.id ?? ""),
        supplier: p.supplier ? { id: String(p.supplier.id), name: p.supplier.name } : undefined,
        description: p.description ?? "",
        price: Number(p.price ?? 0),
        costPrice: Number(p.costPrice ?? p.cost_price ?? 0),
        minStock: Number(p.minStock ?? p.min_stock ?? p.min_quantity ?? p.minQuantity ?? 0),
        unit: p.unit ?? "PCS",
...
export default function ProductsPage() {
    ...
    const load = () => {
        Promise.all([productsApi.getAll(), categoriesApi.getAll(), suppliersApi.getAll()])
            .then(([p, c, s]) => {
                const rawProducts = extractArray<any>(p.data);
    ...
    // Active-only suppliers for dropdowns
    const activeSuppliers = suppliers.filter(
        s => s.isActive === true || s.status === "ACTIVE" || (s.isActive === undefined && s.status === undefined)
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            // Map frontend form to exact backend schema keys & numeric types
            const payload: any = {
                name: form.name.trim(),
                code: form.code.trim(),
                ...
                min_quantity: form.minStock !== "" ? parseInt(form.minStock, 10) : 0,
                unit: form.unit,
                status: form.status,
                category_id: form.categoryId ? parseInt(form.categoryId, 10) : undefined,
                supplier_id: form.supplierId ? parseInt(form.supplierId, 10) : undefined,
            };

            if (!editing) {
                payload.company_id = user?.company_id ? Number(user.company_id) : undefined;
            }
                    ...
                    {filtered.map(p => (
                        <tr key={p.id} className={`border-b border-gray-50 hover:bg-gray-50/50 ${p.status !== "ACTIVE" ? "opacity-60" : ""}`}>
                            <td className="px-5 py-3">
                                ...
                                <p className="text-xs text-gray-400">{p.sku || p.code}{p.barcode ? ` · ${p.barcode}` : ""}</p>
                            </td>
