...
interface StockItem {
    ...
    productName: string; sku: string; branchName: string;
    quantity: number; reservedQuantity: number; minStock: number; status: string;
    product?: { id: string; name: string; sku: string; minStock: number; unit: string };
    branch?: { id: string; name: string };
    stockQty?: number; reservedQty?: number;
}
interface Branch { id: string; name: string; }
interface Product { id: string; name: string; }

// FIX: action types matched exactly to backend enum
const ACTION_TYPES = [
    "ADJUSTMENT",
    "SALE",
    "RESTOCK",
...
export default function StockLevelsPage() {
    ...
    // FIX: assignForm only has fields the backend accepts — removed minStock
    ...
    const load = () => {
        Promise.all([
            ...
            .then(([inv, br, pr]) => {
                const raw = extractArray<StockItem>(inv.data);
                const normalized = raw.map(item => ({
                    ...item,
                    productName: item.productName ?? item.product?.name ?? "—",
                    branchName:  item.branchName  ?? item.branch?.name  ?? "—",
                    quantity:    item.quantity     ?? item.stockQty      ?? 0,
                    reservedQuantity: item.reservedQuantity ?? item.reservedQty ?? 0,
                    minStock:    item.minStock     ?? item.product?.minStock ?? 0,
                }));
                setItems(normalized);
    ...
    // FIX: sends { productId, branchId, stockQty } — exactly what backend expects
    ...
    // FIX: sends { changeQty, action, description } + ?userId= query param (handled in api.ts)
    const handleAdjust = async (e: React.FormEvent) => {
        ...
        try {
            await inventoryApi.adjustStock(showAdjust.branchId, showAdjust.productId, {
                changeQty:   adjustQty,                    // ← was `quantity`
                action:      adjustType,                   // ← was `type`
                description: adjustNote || undefined,      // ← was `note`
            });
        ...
        <div className="p-6">
            ...
            <div className="bg-white rounded-lg border border-gray-100 shadow-sm">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                    {filtered.length === 0 && (
                        <tr><td colSpan={7} className="px-5 py-10 text-center text-gray-400">No stock records</td></tr>
                    )}
                    {filtered.map(item => (
                        <tr key={`${item.branchId}-${item.productId}`} className="border-b border-gray-50 hover:bg-gray-50/50">
                            ...
                            <td className="px-5 py-3"><LevelBar qty={item.quantity} min={item.minStock} /></td>
                            <td className="px-5 py-3"><StatusPill qty={item.quantity} min={item.minStock} /></td>
                            <td className="px-5 py-3">
                                <button onClick={() => openAdjust(item)}
                                        className="text-[#4A8FD4] hover:underline text-sm">Adjust</button>
                            </td>
                        </tr>
                    ))}
                    </tbody>
                    ...
                    <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
                        ...
                        <form onSubmit={handleAssign} className="space-y-4">
                            <div>
                                ...
                                <select value={assignForm.productId}
                            ...
                            {/* Admin picks branch; manager/staff auto-assigned to their branch */}
                            {isAdmin ? (
                                <div>
                                    <label className="block text-xs text-gray-500 mb-1">Branch</label>
                                    <select value={assignForm.branchId}
                            ...
                            {/* FIX: only stockQty — minStock is set on the Product, not here */}
                            <div>
                ...
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
                        ...
                        <form onSubmit={handleAdjust} className="space-y-5">
                            {/* FIX: action type buttons — labels formatted cleanly */}
                            <div>
                            ...
                            {/* FIX: field name is changeQty internally, but UI shows same control */}
                            <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <button type="button" onClick={() => setAdjustQty(q => q - 1)}
                                            className="w-8 h-8 border border-gray-200 rounded flex items-center justify-center text-gray-600 hover:bg-gray-50 text-lg">−</button>
                                    <input type="number"
                                           value={adjustQty}
                            ...
                            {/* FIX: field is `description` in backend, bound to adjustNote locally */}
                            <div>
                                <label className="block text-xs text-gray-500 mb-1">Note (optional)</label>
                                <input value={adjustNote}
                            ...
                            <div className="flex items-center justify-between py-3 border-t border-gray-100">
                                ...
                                <span className={`text-sm font-semibold ${stockAfter < 0 ? "text-red-500" : "text-gray-800"}`}>
                            ...
                            <div className="flex justify-end gap-3">
                                <button type="button" onClick={() => setShowAdjust(null)}
                                        className="px-4 py-2 text-sm border border-gray-200 rounded-md">Cancel</button>
                                <button type="submit" disabled={saving || stockAfter < 0}
