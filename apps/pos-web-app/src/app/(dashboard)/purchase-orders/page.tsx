...
import { purchaseOrdersApi, suppliersApi, branchesApi, productsApi, extractArray, makeScopedApi } from "@/lib/api";
...
interface POItem { id: string; quantity: number; unitCost: number; totalCost: number; product?: { name: string; sku: string; unit: string }; }
interface Invoice { id: string; invoiceNo: string; status: string; totalAmount: number; dueDate?: string; paidAt?: string; }
...
interface Product  { id: string; name: string; sku: string; unit: string; costPrice?: number; status?: string; }
...
export default function PurchaseOrdersPage() {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                ...
                branchId:   createForm.branchId || userBranchId,
    ...
    const handleStatusUpdate = async (id: string, status: string) => {
        try {
            ...
            setSelected(res.data);
            if (status === "RECEIVED") {
                const po = res.data as PO;
                const total = po.items?.reduce((s, i) => s + Number(i.totalCost ?? 0), 0) ?? 0;
        ...
        <div className="p-8 max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-xl font-semibold text-gray-900">
                    Create Invoice — {selected.id.slice(0, 8).toUpperCase()}
                </h1>
                ...
                <div>
                    <p className="text-sm text-[#4A8FD4] mb-1 cursor-pointer hover:underline"
                       onClick={() => setView("list")}>
                        Purchase Orders / {selected.id.slice(0, 8).toUpperCase()}
                    </p>
                    <h1 className="text-xl font-semibold text-gray-900">
                        Purchase Order #{selected.id.slice(0, 8).toUpperCase()}
                    </h1>
                </div>
