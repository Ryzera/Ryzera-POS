...
export default function SettingsPage() {
  ...
  const fetchSettings = useCallback(async () => {
    setLoading(true);
    try {
      const branchQuery = selectedBranchScope !== 'global' ? `?branchId=${selectedBranchScope}` : '';
      ...
      const res = await fetch(`${API_BASE}/sync/settings${branchQuery}`, { headers });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/sync/settings${branchQuery}`, {
        method: 'POST',
        headers,
      ...
      toast.dismiss(tid);
      toast.success(selectedBranchScope === 'global' ? 'Global configuration updated' : `Branch #${selectedBranchScope} configuration updated`);
    } catch (err) {
      toast.dismiss(tid);
    ...
    <div className="max-w-[1400px] mx-auto space-y-10 px-4">
        <div className="flex items-center justify-between mb-10 bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-6">
            {/* Dynamic Multi-Branch Editing Scope */}
