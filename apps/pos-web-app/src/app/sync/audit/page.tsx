                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center text-[10px] font-black uppercase shadow-sm">
                           {log.user_name ? log.user_name.slice(0,1) : ((log.user_id || log.userId) ? 'U' : 'S')}
                        </div>
                        <span className="text-xs font-bold text-slate-600">
                          {log.user_name ? log.user_name : ((log.user_id || log.userId) ? `User ${(log.user_id || log.userId)}` : 'System Engine')}
                        </span>
                      </div>
                    </td>
                    <td className="px-8 py-5">
