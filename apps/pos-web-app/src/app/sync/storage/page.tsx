...
export default function StorageManagerPage() {
  ...
  const injectMockStorageData = (d: any, index: number) => {
    const isCritical = index % 5 === 0;
    return {
      ...d,
      dbSizeMB: Math.max(120, 1500 - (index * 250 % 1000)), // up to 1.5GB
      fragmentation: isCritical ? Math.max(15, 35 - index) : Math.max(1, index % 8),
      oldRecords: Math.max(1000, 50000 - index * 5000),
      status: isCritical ? 'CRITICAL' : (index % 2 === 0 ? 'WARNING' : 'OPTIMAL'),
    };
  };
