...
export default function SyncLayout({ children }: { children: React.ReactNode }) {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/sw.js').then((registration) => {
        ...
        if ('sync' in registration) {
          (registration as any).sync.register('sync-pending-data').catch(console.error);
        }
  ...
  const hasAccess = isAdmin || isManager || isCashier || isInventoryManager;
  ...
  useEffect(() => {
    ...
    if (!isAdmin) {
      ...
      // Monitoring APIs are manager/admin-only; prevent lower roles from reaching
      // pages that would otherwise render after receiving 403 responses.
      const managerMonitoringRoutes = [
        '/sync/dashboard',
        '/sync/queue',
        '/sync/errors',
        '/sync/conflicts',
      ...
      const isUserOnManagerRoute = managerMonitoringRoutes.some(route => pathname.startsWith(route));
      if (!isManager && isUserOnManagerRoute) {
        toast.error('Sync monitoring is restricted to Manager or Admin roles.');
        router.push('/dashboard');
      }
    }
  ...
  const fetchNotifs = async () => {
    try {
      ...
      if (cr.ok) {
        ...
        const count = res.data !== undefined ? (typeof res.data === 'number' ? res.data : res.data?.data) : res;
  ...
  const clearAll = async () => {
    try {
      await authFetch(`${API_BASE}/notifications`, { method: 'DELETE' });
      await fetchNotifs();
  ...
  const navGroups = [
    {
      ...
      show: true,
      items: [
        { label: 'Dashboard', icon: LayoutDashboard, path: '/sync/dashboard', show: isAdmin || isManager },
    ...
    },
    {
      ...
      show: true,
      items: [
        { label: 'Sync Queue', icon: List, path: '/sync/queue', show: isAdmin || isManager },
                  ...
                  <div style={{ maxHeight: '320px', overflowY: 'auto' }}>
                    {sortedNotifs.length === 0 ? (
                      <div style={{ padding: '32px 24px', textAlign: 'center' }}>
                        ...
                        <p style={{ fontSize: '11px', color: '#cbd5e1', marginTop: '4px' }}>Notifications appear here when real system events occur</p>
                      </div>
                    ) : (
                      sortedNotifs.map(n => {
