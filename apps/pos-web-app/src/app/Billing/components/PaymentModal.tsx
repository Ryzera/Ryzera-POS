...
export default function PaymentModal({ onClose }: Props) {
            ...
            try {
                const sale = await createSale({
                    ...
                    items: items.map(({ product, quantity }) => ({
                        ...
                        quantity, unit: 'pcs', unit_price: product.price,
                        cost_price: product.price, discount_percent: 0, tax_percent: taxRate,
                    })),
                });
                await processPayment({
                    sale_id: sale.sale_id, payment_method: paymentMethod,
                    amount_paid: paymentMethod === 'Cash' ? parseFloat(cashReceived) : total,
                    transaction_reference: '',
                });
            } catch {
                // API unreachable — save locally
            }
    ...
    function handleNewSale() { clearCart(); onClose(); }
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            ...
                            {discAmt > 0 && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#16a34a' }}>
                                    <span>Discount ({discount}%)</span><span>− Rs. {discAmt.toLocaleString()}</span>
                                </div>
                            )}
                            {taxAmt > 0 && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#6b7280' }}>
