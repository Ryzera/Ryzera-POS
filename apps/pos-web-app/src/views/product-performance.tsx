            ...
            <div style={{ minWidth: Math.max(data.length * 60, 320) }}>
                <ResponsiveContainer width="100%" height={260}>
                    ...
                    >
                        ...
                        />
                        <Tooltip
                            formatter={(val: number) => [formatNumber(val), 'Units Sold']}
                            contentStyle={{
                                fontSize:        12,
                                borderRadius:    10,
...
export default function ProductPerformanceView() {
    ...
    const showTabs = isSuperAdmin && selectedBranchId === ALL_BRANCHES_SENTINEL;
    ...
    const headerBranchLabel = useMemo(() => {
        ...
        if (user?.branchId) {
            const branch = branches.find(b => String(b.branchId) === String(user.branchId));
    ...
    const effectiveBranchId = useMemo((): string | undefined => {
        if (isBranchManager)                                   return undefined;
                ...
                <div className="flex flex-wrap items-end gap-3 px-5 py-4
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button variant="outline"
                                        className={cn(calTriggerClass, 'w-44', !dateFrom && 'text-gray-400')}>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button variant="outline"
                                        className={cn(calTriggerClass, 'w-44', !dateTo && 'text-gray-400')}>
                                    <CalendarIcon className="mr-2 h-3.5 w-3.5 text-gray-400 flex-shrink-0" />
                                    <span className="truncate">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <Select value={categoryId} onValueChange={setCategoryId}>
                            ...
                            <SelectContent className="rounded-xl z-[200] max-h-64 overflow-y-auto
                                ...
                                <SelectItem value={PP_ALL_CATEGORIES_VALUE}>All Categories</SelectItem>
                                {categories.map(cat => (
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                                    {(perBranchQuery.data?.branches ?? []).map((entry, idx) => (
                                        <PerBranchTopCard key={entry.branchId} entry={entry} index={idx} />
                                    ))}
                                </div>
                ...
                {isGenerated && (!showTabs || activeTab === 'all') && (
                    <div className="space-y-5">
                            ...
                            {cardsQuery.isLoading ? (
                                ...
                                <>
                                    <KpiCard
                                        ...
                                        value={formatNumber(cardsQuery.data?.totalProductsSold ?? 0)}
                                        icon={Package}
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                            <div className="xl:col-span-3 bg-white rounded-2xl border border-gray-100
                                            shadow-sm p-6">
                                <div className="mb-4">
                                    <h2 className="text-[14px] font-bold text-gray-800">Top 10 Products</h2>
                                    <p className="text-[11px] text-gray-400 mt-0.5">
