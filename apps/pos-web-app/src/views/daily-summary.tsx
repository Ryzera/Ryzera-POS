    ...
    return (
      <div className="flex flex-col items-center justify-center gap-2 h-64">
        ...
        <p className="text-sm text-gray-400 font-medium">
          No hourly sales data for this date
        </p>
      </div>
    );
  }
      ...
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            Complete breakdown for the day
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-[13px] text-gray-400 font-medium">
            No summary data recorded for this date
          </p>
        </div>
      ) : (
...
function PlBreakdownSection({
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="flex flex-col items-center justify-center py-8 gap-2">
          ...
          <p className="text-[12px] text-gray-400 font-medium">
            No P&amp;L data available for this date
          </p>
        </div>
      </div>
    );
  ...
  const displayValue = (key: string, value: number): string => {
    ...
    if (key === "taxCollected") return `+${formatCurrency(value)}`;
    return formatCurrency(value);
  };
      ...
      <div className="space-y-0">
        {PL_ROWS.map((row, i) => {
          ...
          return (
            ...
            >
              <span
                className={`text-[13px] ${isLast ? "font-bold text-gray-900" : "text-gray-600"}`}
              >
                {row.label}
              </span>
              <span
          ...
          <div className="space-y-3">
            {(
              [
                ...
                {
                  label: "Transactions",
                  value: formatNumber(kpi.transactions),
                },
...
function PerBranchDetailSection({
  ...
  const { data: hourlyResp, isLoading: hourlyLoading } = useHourlySales(
    branchParams,
    enabled,
  );
  const { data: pmResp, isLoading: pmLoading } = useDailyPaymentMethods(
    branchParams,
    enabled,
  );
  const { data: detailsResp, isLoading: detailsLoading } =
    useDailySummaryDetails(branchParams, enabled);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span
          className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${badge}`}
        >
          Branch {index + 1}
        </span>
      </div>
    ...
    <div className="space-y-10">
      <div>
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <span
            className="text-[11px] bg-gray-100 text-gray-500 font-semibold
          ...
          {withData > 0 && (
            <span
              className="text-[11px] bg-emerald-100 text-emerald-700 font-semibold
                                         px-2.5 py-1 rounded-full"
            >
              {withData} with data
            </span>
          ...
          {withoutData > 0 && (
            <span
              className="text-[11px] bg-orange-100 text-orange-700 font-semibold
                                         px-2.5 py-1 rounded-full"
            >
              {withoutData} no data
            </span>
...
export default function DailySummaryPage() {
  ...
  const [selectedBranchId, setSelectedBranchId] =
    useState<string>(ALL_BRANCHES_VALUE);
  ...
  // ── Queries ───────────────────────────────────────────────
  const { data: cardsResp, isLoading: cardsLoading } = useDailySummaryCards(
    queryParams ?? {},
    isAllEnabled,
  );
  const { data: hourlyResp, isLoading: hourlyLoading } = useHourlySales(
  ...
  const isLoading =
    cardsLoading || hourlyLoading || pmLoading || detailsLoading;
  ...
  const handleExport = useCallback(
    async (type: "csv" | "pdf") => {
      ...
      try {
        ...
        downloadBlob(
          blob,
          `daily-summary-${queryParams.date ?? "all"}.${type}`,
        );
      } catch (err) {
  ...
  const headerBranchLabel = useMemo(() => {
    ...
    if (user?.branch_id) {
      const b = branchesRaw.find((br) => br.branchId === user.branch_id);
      return b?.name ?? `Branch ${user.branch_id}`;
    }
    return null;
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col">
      ...
      <div className="flex-1 px-8 py-6 space-y-5 overflow-y-auto">
        ...
        {isAdmin && generated && selectedBranchId === ALL_BRANCHES_VALUE && (
          <div
        ...
        {!generated && (
          <div className="flex flex-col items-center justify-center py-36 gap-5">
            ...
            <p className="text-[14px] text-gray-400 font-medium text-center max-w-xs">
              ...
              <span className="text-blue-600 font-bold">Generate</span> to load
              the summary.
            </p>
          </div>
        )}
          ...
          <div className="space-y-5">
              ...
              {cardsLoading ? (
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[13px] text-gray-400 mt-1">
                      No recorded sales for the selected date. Try a different
                      date.
                    </p>
                  </div>
                </div>
                ...
                <>
                  ...
                  <KpiCard
                    label="Items Sold"
                    value={formatNumber(kpi.itemsSold)}
                    icon={Package}
                    iconBg="bg-violet-50"
            ...
            {kpi && (
              <>
                <DailySummaryTable
                  detailsResp={
                    detailsResp as DailySummaryDetailsResponse | undefined
                  }
                  isLoading={detailsLoading}
                  onExportCsv={() => handleExport("csv")}
                  onExportPdf={() => handleExport("pdf")}
        ...
        {/* ══ PER BRANCH TAB ════════════════════════ */}
        {generated && activeTab === "per-branch" && isAdmin && (
          <PerBranchTab
