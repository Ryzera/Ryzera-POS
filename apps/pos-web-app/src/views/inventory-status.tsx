            ...
            <tbody className="divide-y divide-gray-50">
              {rows.map((row, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-3.5 text-[12px] text-gray-700 font-medium">
                    {row.currentStock}
                  </td>
...
function PerBranchSection({ entry, baseParams }: PerBranchSectionProps) {
  const [exporting, setExporting] = useState<ExportType>(null);

  const exportParams = useMemo(
    () => ({ ...baseParams, invBranchId: String(entry.branch.id) }),
...
export default function InventoryStatusPage() {
  ...
  const queryParams = useMemo((): InventoryStatusQueryParams => {
    const effectiveBranchId = isAdmin
      ...
      : (user?.branch_id != null ? String(user.branch_id) : undefined);
  ...
  }, [selectedCategory, selectedStockStatus, selectedBranchId, isAdmin, user?.branch_id]);

  // ── Derived booleans ──────────────────────────────────────────────────────
  const isViewingAllBranches = isAdmin && !selectedBranchId;
  const showTabs             = isViewingAllBranches && generated;

  // ── Data hooks ────────────────────────────────────────────────────────────
  const isAllEnabled  = generated && activeTab === 'all';
  const isPerBEnabled = generated && activeTab === 'per-branch' && isAdmin;
  ...
  const headerBranchLabel = useMemo((): string | null => {
    ...
    if (user?.branch_id) {
      const branch = branchesRaw.find(b => b.branchId === user.branch_id);
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <NotificationsDropdown branchId={isAdmin ? undefined : (user?.branch_id ?? undefined)} />
          </div>
        </div>
      </div>
